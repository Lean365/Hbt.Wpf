...
public class BaseRepository<TEntity> : IBaseRepository<TEntity> where TEntity : class, new()
{
    ...
    public async Task<TResult?> GetMedianAsync<TResult>(Expression<Func<TEntity, TResult>> selector, Expression<Func<TEntity, bool>>? condition = null) where TResult : struct
    {
        ...
        // 获取表名
        var tableName = _dbContext.Db.EntityMaintenance.GetTableName(typeof(TEntity));
        ...
        // 获取实体信息
        var entityInfo = _dbContext.Db.EntityMaintenance.GetEntityInfo(typeof(TEntity));
        ...
        if (!string.IsNullOrWhiteSpace(whereSql))
        {
            ...
            if (whereIndex >= 0)
            {
                whereClause = whereSql.Substring(whereIndex);
            }
        }
        ...
        switch (dbType)
        {
            case DbType.SqlServer:
                // SQL Server 2012+ 使用 PERCENTILE_CONT
                medianSql = $@"
            ...
            case DbType.PostgreSQL:
                // PostgreSQL 9.4+ 使用 PERCENTILE_CONT
                medianSql = $@"
                    SELECT PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY ""{columnName}"") AS Median
                    FROM ""{tableName}""
                    {whereClause}";
    ...
    public async Task<int> DeleteAsync(object id)
    {
        if (typeof(TEntity).IsSubclassOf(typeof(BaseEntity)))
        {
            ...
            return await _dbContext.Db.Updateable<TEntity>()
                ...
                .Where($"{primaryKeyColumnName} = @id", new { id })
                .ExecuteCommandAsync();
        }
