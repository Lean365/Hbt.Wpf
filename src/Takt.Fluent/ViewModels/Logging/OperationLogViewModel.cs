...
public partial class OperationLogViewModel : ObservableObject
{
    ...
    public ObservableCollection<OperationLogDto> OperationLogs { get; } = new();

    [ObservableProperty]
    private OperationLogDto? _selectedOperationLog;
    ...
    private async Task LoadAsync()
    {
        ...
        try
        {
            _operLog?.Information("[OperationLogView] Load operation logs: pageIndex={PageIndex}, pageSize={PageSize}, keyword={Keyword}", PageIndex, PageSize, Keyword);
            ...
            if (!result.Success || result.Data == null)
            {
                OperationLogs.Clear();
                TotalCount = 0;
                ErrorMessage = result.Message ?? GetTranslation("Logging.OperationLog.LoadFailed", "加载操作日志数据失败");
                return;
            }
        ...
        {
            _operLog?.Error(ex, "[OperationLogView] Load operation logs failed");
            ErrorMessage = GetTranslation("Logging.OperationLog.LoadFailed", "加载操作日志数据失败");
        }
        finally
        {
            IsLoading = false;
    ...
    {
        // TODO: 实现导出功能
        _operLog?.Information("[OperationLogView] Export operation logs");
    }
}
