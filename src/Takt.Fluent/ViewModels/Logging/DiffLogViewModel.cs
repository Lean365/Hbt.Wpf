...
public partial class DiffLogViewModel : ObservableObject
{
    ...
    private async Task LoadAsync()
    {
        ...
        try
        {
            ...
            if (!result.Success || result.Data == null)
            {
                DiffLogs.Clear();
                TotalCount = 0;
                ErrorMessage = result.Message ?? _localizationManager.GetString("logging.difflog.loadfailed");
                return;
            }
        ...
        catch (Exception ex)
        {
            ...
            ErrorMessage = _localizationManager.GetString("logging.difflog.loadfailed");
        }
        finally
        {
            IsLoading = false;
    ...
    private async Task QueryAsync(QueryContext context)
    {
        ...
        _operLog?.Information("[DiffLogView] 执行查询操作，操作人={Operator}, 关键词={Keyword}, 页码={PageIndex}, 每页数量={PageSize}",
    ...
    {
        var operatorName = UserContext.Current?.IsAuthenticated == true ? UserContext.Current.Username ?? "Takt365" : "Takt365";
        _operLog?.Information("[DiffLogView] 执行重置操作，操作人={Operator}", operatorName);
    ...
    {
        var operatorName = UserContext.Current?.IsAuthenticated == true ? UserContext.Current.Username ?? "Takt365" : "Takt365";
        _operLog?.Information("[DiffLogView] 分页变化，操作人={Operator}, 页码={PageIndex}, 每页数量={PageSize}",
    ...
    private async Task ExportAsync()
    {
        ...
        {
            var operatorName = UserContext.Current?.IsAuthenticated == true ? UserContext.Current.Username ?? "Takt365" : "Takt365";
            _operLog?.Information("[DiffLogView] 开始导出差异日志，操作人={Operator}", operatorName);
            ...
            var result = await _diffLogService.ExportAsync(query);
            ...
            if (result.Success && result.Data.content != null)
            {
                ...
                _operLog?.Information("[DiffLogView] 导出差异日志成功，操作人={Operator}, 文件路径={FilePath}, 记录数={Count}",
            ...
            else
            {
                ...
                _operLog?.Warning("[DiffLogView] 导出差异日志失败，操作人={Operator}, 错误={Message}",
                    operatorName, result.Message ?? "未知错误");
            }
        }
        catch (Exception ex)
        {
            _operLog?.Error(ex, "[DiffLogView] 导出差异日志异常");
    ...
    private async Task ClearAsync(object? parameter)
    {
        ...
        {
            IsLoading = true;
            _operLog?.Information("[DiffLogView] Start cleaning up logs, range={Range}", range);

            var loadingMessage = _localizationManager.GetString("logging.cleanup.inprogress");
            TaktMessageManager.ShowInformation(loadingMessage);

            var result = await _logCleanupService.CleanupLogsAsync(LogType.DiffLog, range);
            ...
            if (result.Success && result.Data != null)
            {
                ...
                _operLog?.Information("[DiffLogView] Log cleanup successful: {RecordCount} records",
                    result.Data.CleanedDatabaseLogCount);
            ...
            else
            {
                ...
                _operLog?.Warning("[DiffLogView] Log cleanup failed: {Message}", result.Message ?? "未知错误");
            }
        }
        catch (Exception ex)
        {
            _operLog?.Error(ex, "[DiffLogView] Log cleanup exception");
            var errorMessage = string.Format(
                _localizationManager.GetString("logging.cleanup.failed"),
                ex.Message);
            TaktMessageManager.ShowError(errorMessage);
