...
/// 操作日志视图模型
/// </summary>
public partial class OperLogViewModel : ObservableObject
{
    private readonly IOperLogService _operationLogService;
    private readonly ILogCleanupService _logCleanupService;
    private readonly ILocalizationManager _localizationManager;
    private readonly OperLogManager? _operLog;

    public ObservableCollection<OperLogDto> OperLogs { get; } = new();

    [ObservableProperty]
    private OperLogDto? _selectedOperLog;
    ...
    public OperLogViewModel(
        IOperLogService operationLogService,
    ...
    private async Task LoadAsync()
    {
        ...
        try
        {
            _operLog?.Information("[OperLogView] Load operation logs: pageIndex={PageIndex}, pageSize={PageSize}, keyword={Keyword}", PageIndex, PageSize, Keyword);
            ...
            if (!result.Success || result.Data == null)
            {
                OperLogs.Clear();
                TotalCount = 0;
                ErrorMessage = result.Message ?? _localizationManager.GetString("logging.operlog.loadfailed");
                return;
            }
        ...
        {
            _operLog?.Error(ex, "[OperLogView] Load operation logs failed");
            ErrorMessage = _localizationManager.GetString("logging.operlog.loadfailed");
        }
        finally
        {
            IsLoading = false;
    ...
    private async Task QueryAsync(QueryContext context)
    {
        ...
        _operLog?.Information("[OperLogView] 执行查询操作，操作人={Operator}, 关键词={Keyword}, 页码={PageIndex}, 每页数量={PageSize}", 
    ...
    {
        var operatorName = UserContext.Current?.IsAuthenticated == true ? UserContext.Current.Username ?? "Takt365" : "Takt365";
        _operLog?.Information("[OperLogView] 执行重置操作，操作人={Operator}", operatorName);
    ...
    {
        var operatorName = UserContext.Current?.IsAuthenticated == true ? UserContext.Current.Username ?? "Takt365" : "Takt365";
        _operLog?.Information("[OperLogView] 分页变化，操作人={Operator}, 页码={PageIndex}, 每页数量={PageSize}", 
    ...
    private async Task ExportAsync()
    {
        ...
        {
            var operatorName = UserContext.Current?.IsAuthenticated == true ? UserContext.Current.Username ?? "Takt365" : "Takt365";
            _operLog?.Information("[OperLogView] 开始导出操作日志，操作人={Operator}", operatorName);
            ...
            if (result.Success && result.Data.content != null)
            {
                ...
                _operLog?.Information("[OperLogView] 导出操作日志成功，操作人={Operator}, 文件路径={FilePath}, 记录数={Count}", 
            ...
            else
            {
                ...
                _operLog?.Warning("[OperLogView] 导出操作日志失败，操作人={Operator}, 错误={Message}", 
                    operatorName, result.Message ?? "未知错误");
            }
        }
        catch (Exception ex)
        {
            _operLog?.Error(ex, "[OperLogView] 导出操作日志异常");
    ...
    private async Task ClearAsync(object? parameter)
    {
        ...
        {
            IsLoading = true;
            _operLog?.Information("[OperLogView] Start cleaning up logs, range={Range}", range);

            var loadingMessage = _localizationManager.GetString("logging.cleanup.inprogress");
            TaktMessageManager.ShowInformation(loadingMessage);

            var result = await _logCleanupService.CleanupLogsAsync(LogType.OperLog, range);
            ...
            if (result.Success && result.Data != null)
            {
                ...
                _operLog?.Information("[OperLogView] Log cleanup successful: {RecordCount} records", 
                    result.Data.CleanedDatabaseLogCount);
            ...
            else
            {
                ...
                _operLog?.Warning("[OperLogView] Log cleanup failed: {Message}", result.Message ?? "未知错误");
            }
        }
        catch (Exception ex)
        {
            _operLog?.Error(ex, "[OperLogView] Log cleanup exception");
            var errorMessage = string.Format(
                _localizationManager.GetString("logging.cleanup.failed"),
                ex.Message);
            TaktMessageManager.ShowError(errorMessage);
