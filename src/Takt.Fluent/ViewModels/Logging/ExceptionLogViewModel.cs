...
/// 异常日志视图模型
/// </summary>
public partial class ExceptionLogViewModel : ObservableObject
{
    private readonly IExceptionLogService _exceptionLogService;
    private readonly ILocalizationManager _localizationManager;
    private readonly OperLogManager? _operLog;

    public ObservableCollection<ExceptionLogDto> ExceptionLogs { get; } = new();

    [ObservableProperty]
    private ExceptionLogDto? _selectedExceptionLog;
    ...
    {
        _exceptionLogService = exceptionLogService ?? throw new ArgumentNullException(nameof(exceptionLogService));
    ...
    private async Task LoadAsync()
    {
        ...
        try
        {
            _operLog?.Information("[ExceptionLogView] Load exception logs: pageIndex={PageIndex}, pageSize={PageSize}, keyword={Keyword}", PageIndex, PageSize, Keyword);

            // 构建查询DTO
            var query = new ExceptionLogQueryDto
            {
                PageIndex = PageIndex,
                PageSize = PageSize,
            ...
            if (!result.Success || result.Data == null)
            {
                ExceptionLogs.Clear();
                TotalCount = 0;
                ErrorMessage = result.Message ?? GetTranslation("Logging.ExceptionLog.LoadFailed", "加载异常日志数据失败");
                return;
            }
        ...
        {
            _operLog?.Error(ex, "[ExceptionLogView] Load exception logs failed");
            ErrorMessage = GetTranslation("Logging.ExceptionLog.LoadFailed", "加载异常日志数据失败");
        }
        finally
        {
            IsLoading = false;
    ...
    {
        // TODO: 实现导出功能
        _operLog?.Information("[ExceptionLogView] Export exception logs");
    }
}
