...
public partial class MainWindowViewModel : ObservableObject
{
    ...
    public void Settings()
    {
        ...
        try
        {
            ...
            var title = _localizationManager.GetString("menu.settings") ?? "用户设置";
    ...
    {
        return _localizationManager.GetString("application.title") ?? "Takt SMEs";
    }
            ...
            foreach (var tab in DocumentTabs)
            {
                if (tab.MenuItem != null)
                {
                    ...
                    var newTitle = !string.IsNullOrWhiteSpace(titleKey)
                        ? _localizationManager.GetString(titleKey) ?? tab.MenuItem.MenuName ?? "未命名"
    ...
    private DocumentTabItem CreateOrActivateTabCore(MenuDto menuItem, object content, string viewTypeName)
    {
        ...
        try
        {
            ...
            var title = !string.IsNullOrWhiteSpace(titleKey) 
                ? _localizationManager.GetString(titleKey) ?? menuItem.MenuName ?? "未命名"
    ...
    public void ShowUserInfo()
    {
        System.Windows.Application.Current.Dispatcher.Invoke(() =>
        {
            ...
            if (mainWindow != null)
            {
                var userInfoWindow = App.Services?.GetService<Views.Identity.UserComponent.UserInfo>();
