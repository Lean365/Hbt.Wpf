...
public partial class ImportTableViewModel : ObservableObject
{
    ...
    private void Query()
    {
        ...
        try
        {
            ...
            {
                // 如果数据未加载，提示用户
                var message = _localizationManager.GetString("Generator.ImportTable.DataNotLoaded") ?? "数据尚未加载，请稍候";
    ...
    private async Task ImportFromTableAsync()
    {
        ...
        {
            TaktMessageManager.ShowWarning(_localizationManager.GetString("Generator.ImportTable.NoSelection") ?? "请至少选择一个表");
            return;
        }
        ...
        try
        {
            ...
            var message = string.Format(
                _localizationManager.GetString("common.success.import") ?? "导入成功：成功 {0} 条，失败 {1} 条",
                success, fail);
