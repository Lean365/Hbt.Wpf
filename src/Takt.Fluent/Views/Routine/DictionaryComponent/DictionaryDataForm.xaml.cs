...
// 文件名称：DictionaryDataForm.xaml.cs
// 创建时间：2025-01-20
...
/// 字典数据表单窗口
/// </summary>
public partial class DictionaryDataForm : Window
{
    private readonly ILocalizationManager? _localizationManager;
    private DictionaryDataFormViewModel? _viewModel;

    /// <summary>
    /// 初始化字典数据表单窗口
    /// </summary>
    /// <param name="vm">字典数据表单视图模型</param>
    /// <param name="localizationManager">本地化管理器</param>
    public DictionaryDataForm(DictionaryDataFormViewModel vm, ILocalizationManager? localizationManager = null)
    {
        InitializeComponent();
    ...
    {
        // 统计字段数量：类型代码、数据代码、数据名称、数据值、扩展数据、CSS类名、列表CSS类名、排序号、备注 = 9个
        int normalFieldCount = 6; // 类型代码、数据代码、数据名称、数据值、CSS类名、列表CSS类名、排序号
        double normalFieldsHeight = CalculateHeightByFieldCount(normalFieldCount);

        // 扩展数据和备注字段高度
        const double extendedDataTextBoxHeight = 80;
        const double extendedDataFieldHeight = extendedDataTextBoxHeight + 56;
        ...
        const double specialFieldsSpacing = 16.0; // 扩展数据和备注之间的间距

        double totalHeight = normalFieldsHeight + extendedDataFieldHeight + remarksFieldHeight + normalFieldsSpacing + specialFieldsSpacing + stackPanelMargin;
