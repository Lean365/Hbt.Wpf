...
// 文件名称：SerialInboundView.xaml.cs
// 创建时间：2025-01-20
...
public partial class SerialInboundView : UserControl
{
    public SerialInboundViewModel ViewModel { get; }

    public SerialInboundView(SerialInboundViewModel viewModel)
    {
        InitializeComponent();
    ...
    private void InboundDataGrid_Loaded(object sender, System.Windows.RoutedEventArgs e)
    {
        if (sender is TaktDataGrid dataGrid)
        {
            dataGrid.SelectedItemsCountChanged += InboundDataGrid_SelectedItemsCountChanged;
        }
    }

    private void InboundDataGrid_SelectedItemsCountChanged(object? sender, int count)
    {
        // 可以在这里处理选中项数量变化
    }
}
