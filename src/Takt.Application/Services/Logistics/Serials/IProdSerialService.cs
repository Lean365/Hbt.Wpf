...
/// 产品序列号服务接口
/// </summary>
public interface IProdSerialService
{
    /// <summary>
    ...
    Task<Result<PagedResult<ProdSerialDto>>> GetListAsync(ProdSerialQueryDto query);

    /// <summary>
    /// 根据ID获取产品序列号
    /// </summary>
    Task<Result<ProdSerialDto>> GetByIdAsync(long id);

    /// <summary>
    /// 创建产品序列号
    /// </summary>
    Task<Result<long>> CreateAsync(ProdSerialCreateDto dto);

    /// <summary>
    /// 更新产品序列号
    /// </summary>
    Task<Result> UpdateAsync(ProdSerialUpdateDto dto);
    ...
    Task<Result<(string fileName, byte[] content)>> ExportAsync(ProdSerialQueryDto? query = null, string? sheetName = null, string? fileName = null);
