...
public class GenColumnService : IGenColumnService
{
        ...
        try
        {
            ...
            if (result > 0)
            {
                ...
                var operatorName = Takt.Common.Context.UserContext.Current.IsAuthenticated
                    ...
                    : "System";
                var requestParams = System.Text.Json.JsonSerializer.Serialize(new { TableName = dto.TableName, ColumnName = dto.ColumnName });
                _operLog?.Create("GenColumn", entity.Id.ToString(), operatorName, "Generator.GenColumnView", requestParams, (int)stopwatch.ElapsedMilliseconds);
                return Result<long>.Ok(entity.Id);
            }

            return Result<long>.Fail("创建失败");
        }
        catch (Exception ex)
        {
            stopwatch.Stop();
    ...
    /// 批量创建代码生成列配置
    /// </summary>
    public async Task<Result<(int success, int fail, List<string> failMessages)>> CreateRangeAsync(List<GenColumnCreateDto> dtos)
    {
        var stopwatch = System.Diagnostics.Stopwatch.StartNew();
        try
        {
            ...
            {
                var result = await _genColumnRepository.CreateRangeAsync(entitiesToCreate);
                successCount = result;
                ...
                if (result > 0)
                {
                    ...
                    {
                        var requestParams = System.Text.Json.JsonSerializer.Serialize(new { TableName = entity.TableName, ColumnName = entity.ColumnName });
                        _operLog?.Create("GenColumn", entity.Id.ToString(), operatorName, "Generator.GenColumnView", requestParams, (int)stopwatch.ElapsedMilliseconds);
                    }
                }
            }
    ...
    public async Task<Result> UpdateAsync(GenColumnUpdateDto dto)
    {
        ...
        try
        {
            ...
            if (result > 0)
            {
                ...
                var requestParams = System.Text.Json.JsonSerializer.Serialize(new { Id = dto.Id, TableName = dto.TableName, ColumnName = dto.ColumnName });
                _operLog?.Update("GenColumn", dto.Id.ToString(), operatorName, changes, "Generator.GenColumnView", requestParams, (int)stopwatch.ElapsedMilliseconds);
                return Result.Ok();
            }
        ...
        try
        {
            ...
            if (result > 0)
            {
                ...
                var operatorName = Takt.Common.Context.UserContext.Current.IsAuthenticated
                    ? Takt.Common.Context.UserContext.Current.Username
                    : "System";
                var requestParams = System.Text.Json.JsonSerializer.Serialize(new { Id = id });
                _operLog?.Delete("GenColumn", id.ToString(), operatorName, "Generator.GenColumnView", requestParams, (int)stopwatch.ElapsedMilliseconds);
                return Result.Ok();
            }
    ...
    public async Task<Result> DeleteRangeAsync(List<long> ids)
    {
        ...
        try
        {
            ...
            var operatorName = Takt.Common.Context.UserContext.Current.IsAuthenticated
                ? Takt.Common.Context.UserContext.Current.Username
                : "System";
            var requestParams = System.Text.Json.JsonSerializer.Serialize(new { Ids = ids });
            _operLog?.Delete("GenColumn", string.Join(",", ids), operatorName, "Generator.GenColumnView", requestParams, (int)stopwatch.ElapsedMilliseconds);
        ...
        try
        {
            ...
            foreach (var entity in entities.Items.Take(3)) // 只记录前3条作为示例
            {
                _appLog.Debug("实体数据: Id={Id}, TableName={TableName}, ColumnName={ColumnName}, PropertyName={PropertyName}, DataType={DataType}, ColumnDataType={ColumnDataType}, OrderNum={OrderNum}, IsQuery={IsQuery}, QueryType={QueryType}, FormControlType={FormControlType}, DictType={DictType}",
                    entity.Id, entity.TableName, entity.ColumnName, entity.PropertyName, entity.DataType, entity.ColumnDataType, entity.OrderNum, entity.IsQuery, entity.QueryType, entity.FormControlType, entity.DictType);
            }
            ...
            foreach (var dto in dtos.Take(3)) // 只记录前3条作为示例
            {
                _appLog.Debug("DTO 数据: Id={Id}, TableName={TableName}, ColumnName={ColumnName}, PropertyName={PropertyName}, DataType={DataType}, ColumnDataType={ColumnDataType}, OrderNum={OrderNum}, IsQuery={IsQuery}, QueryType={QueryType}, FormControlType={FormControlType}, DictType={DictType}",
                    dto.Id, dto.TableName, dto.ColumnName, dto.PropertyName, dto.DataType, dto.ColumnDataType, dto.OrderNum, dto.IsQuery, dto.QueryType, dto.FormControlType, dto.DictType);
            }
    ...
    public async Task<Result<(int success, int fail)>> ImportAsync(Stream fileStream, string? sheetName = null)
    {
        ...
        try
        {
            ...
            var operatorName = Takt.Common.Context.UserContext.Current.IsAuthenticated
                ? Takt.Common.Context.UserContext.Current.Username
                : "System";
            var requestParams = System.Text.Json.JsonSerializer.Serialize(new { Total = dtos.Count, Success = success, Fail = fail });
            _operLog?.Import("GenColumn", success, operatorName);
            _operLog?.Update("GenColumn", "Import", operatorName, $"导入了 {success} 条数据，失败 {fail} 条", "Generator.GenColumnView", requestParams, (int)stopwatch.ElapsedMilliseconds);
