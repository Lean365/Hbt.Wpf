...
public class GenTableService : IGenTableService
{
        ...
        try
        {
            ...
            if (result > 0)
            {
                ...
                var operatorName = Takt.Common.Context.UserContext.Current.IsAuthenticated
                    ...
                    : "System";
                var requestParams = System.Text.Json.JsonSerializer.Serialize(new { TableName = dto.TableName });
                _operLog?.Create("GenTable", entity.Id.ToString(), operatorName, "Generator.GenTableView", requestParams, (int)stopwatch.ElapsedMilliseconds);
    ...
    public async Task<Result> UpdateAsync(GenTableUpdateDto dto)
    {
        ...
        try
        {
            ...
            if (result > 0)
            {
                ...
                var requestParams = System.Text.Json.JsonSerializer.Serialize(new { Id = dto.Id, TableName = dto.TableName });
                _operLog?.Update("GenTable", dto.Id.ToString(), operatorName, changes, "Generator.GenTableView", requestParams, (int)stopwatch.ElapsedMilliseconds);
                return Result.Ok();
            }
    ...
    public async Task<Result> DeleteAsync(long id)
    {
        ...
        try
        {
            ...
            if (result > 0)
            {
                ...
                _operLog?.Delete("GenTable", id.ToString(), operatorName, "Generator.GenTableView", requestParams, (int)stopwatch.ElapsedMilliseconds);
                return Result.Ok();
            }
    ...
    public async Task<Result<(int success, int fail)>> ImportAsync(Stream fileStream, string? sheetName = null)
    {
        ...
        try
        {
            ...
            _operLog?.Import("GenTable", success, operatorName);
            _operLog?.Update("GenTable", "Import", operatorName, $"导入了 {success} 条数据，失败 {fail} 条", "Generator.GenTableView", requestParams, (int)stopwatch.ElapsedMilliseconds);
