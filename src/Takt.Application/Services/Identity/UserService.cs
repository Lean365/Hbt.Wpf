        ...
        try
        {
            ...
            if (result > 0)
            {
                ...
                var operatorName = Takt.Common.Context.UserContext.Current.IsAuthenticated 
                    ...
                    : "System";
                var requestParams = System.Text.Json.JsonSerializer.Serialize(new { Username = dto.Username, RealName = dto.RealName });
                _operLog?.Create("User", user.Id.ToString(), operatorName, "Identity.UserView", requestParams, (int)stopwatch.ElapsedMilliseconds);
        ...
        try
        {
            ...
            if (result > 0)
            {
                ...
                var operatorName = Takt.Common.Context.UserContext.Current.IsAuthenticated 
                    ? Takt.Common.Context.UserContext.Current.Username 
                    : "System";
                var changes = $"Username: {oldUser.Username} -> {dto.Username}, RealName: {oldUser.RealName} -> {dto.RealName}";
                var requestParams = System.Text.Json.JsonSerializer.Serialize(new { UserId = dto.Id, Username = dto.Username });
                _operLog?.Update("User", dto.Id.ToString(), operatorName, changes, "Identity.UserView", requestParams, (int)stopwatch.ElapsedMilliseconds);
                return Result.Ok();
            }
        ...
        try
        {
            ...
            if (result > 0)
            {
                ...
                var operatorName = Takt.Common.Context.UserContext.Current.IsAuthenticated 
                    ? Takt.Common.Context.UserContext.Current.Username 
                    : "System";
                var requestParams = System.Text.Json.JsonSerializer.Serialize(new { UserId = id });
                _operLog?.Delete("User", id.ToString(), operatorName, "Identity.UserView", requestParams, (int)stopwatch.ElapsedMilliseconds);
                return Result.Ok();
            }
        ...
        try
        {
            ...
            if (rows > 0)
            {
                ...
                var operatorName = Takt.Common.Context.UserContext.Current.IsAuthenticated 
                    ? Takt.Common.Context.UserContext.Current.Username 
                    : "System";
                var requestParams = System.Text.Json.JsonSerializer.Serialize(new { UserId = dto.UserId });
                _operLog?.Update("User", dto.UserId.ToString(), operatorName, "修改密码", "Identity.UserView", requestParams, (int)stopwatch.ElapsedMilliseconds);
        ...
        try
        {
            ...
            if (rows > 0)
            {
                ...
                var operatorName = Takt.Common.Context.UserContext.Current.IsAuthenticated 
                    ...
                    : "System";
                var requestParams = System.Text.Json.JsonSerializer.Serialize(new { UserId = dto.UserId });
                _operLog?.Update("User", dto.UserId.ToString(), operatorName, "重置密码", "Identity.UserView", requestParams, (int)stopwatch.ElapsedMilliseconds);
        ...
        try
        {
            ...
            if (result > 0)
            {
                ...
                var operatorName = Takt.Common.Context.UserContext.Current.IsAuthenticated 
                    ? Takt.Common.Context.UserContext.Current.Username 
                    : "System";
                var requestParams = System.Text.Json.JsonSerializer.Serialize(new { UserId = dto.Id, Status = dto.Status });
                _operLog?.Update("User", dto.Id.ToString(), operatorName, $"修改状态为 {dto.Status}", "Identity.UserView", requestParams, (int)stopwatch.ElapsedMilliseconds);
    ...
    public async Task<Result<(int success, int fail)>> ImportAsync(Stream fileStream, string? sheetName = null)
    {
        ...
        try
        {
            ...
            var operatorName = Takt.Common.Context.UserContext.Current.IsAuthenticated 
                ? Takt.Common.Context.UserContext.Current.Username 
                : "System";
            var requestParams = System.Text.Json.JsonSerializer.Serialize(new { Total = userDtos.Count, Success = success, Fail = fail });
            _operLog?.Import("User", success, operatorName);
            // 使用 Update 方法记录导入操作详情
            _operLog?.Update("User", "Import", operatorName, $"导入了 {success} 条用户数据，失败 {fail} 条", "Identity.UserView", requestParams, (int)stopwatch.ElapsedMilliseconds);
    ...
    public async Task<Result> AssignRolesAsync(long userId, List<long> roleIds)
    {
        ...
        try
        {
            ...
            var operatorName = Takt.Common.Context.UserContext.Current.IsAuthenticated 
                ? Takt.Common.Context.UserContext.Current.Username 
                : "System";
            var requestParams = System.Text.Json.JsonSerializer.Serialize(new { UserId = userId, RoleIds = roleIds, DeletedCount = rolesToDelete.Count, AddedCount = rolesToAdd.Count });
            var changes = $"删除 {rolesToDelete.Count} 个角色，添加 {rolesToAdd.Count} 个角色";
            _operLog?.Update("User", userId.ToString(), operatorName, $"分配角色：{changes}", "Identity.UserView", requestParams, (int)stopwatch.ElapsedMilliseconds);
